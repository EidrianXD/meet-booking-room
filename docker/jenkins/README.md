...
Em ~2 min (mais lento na primeira vez por causa do `jenkins-plugin-cli`),
abra http://localhost:8081 e faça login com `JENKINS_ADMIN_ID` /
`JENKINS_ADMIN_PASSWORD` definidos no `.env`.
...
| ID | Tipo | Valor |
|---|---|---|
| `ghcr-pat` | Username/Password | username = login GitHub (`EidrianXD`), password = PAT com escopo `write:packages` |
| `jwt-secret-prod` | Secret text | JWT secret para o deploy "prod" — gere com `openssl rand -hex 32` |
...
- **DooD (Docker outside of Docker):** o container do Jenkins fala com o
  daemon do host via `/var/run/docker.sock`. As builds, scans e pushes
  rodam no Docker do laptop, não em um daemon aninhado. Compromisso
  consciente: o container tem acesso ao Docker do host (privilégio
  elevado equivalente a root). Aceitável em ambiente local; em produção
  use agentes remotos isolados ou kaniko.
- **Setup wizard desligado + JCasC:** admin user vem do YAML, sem etapa
  manual de "Unlock Jenkins" / "Customize". Reprodutível, idempotente.
- **Polling SCM (1 min):** Jenkins consulta o GitHub a cada minuto.
  Trade-off conhecido vs. webhook: latência mas zero dependência de IP
  público / túnel ngrok.
- **Deploy em porta 8090 (não 8080):** evita colisão com `docker compose
  ...
  (separado de qualquer `docker-compose` rodando à mão).
